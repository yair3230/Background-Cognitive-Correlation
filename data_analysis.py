...
def calculate_lisas(df):
    ...
    if correct_trials.empty: return np.nan
    ...
    if spe == 0 or np.isnan(rt_std): return rt_mean
...
def standardize_scores(df):
    df['z'] = df.groupby('task')['lisas'].transform(
        lambda x: (x - x.mean()) / x.std() * -1
    )
    return df

# Reorogenizing data (subject focus)
...
def calculate_metrics(pivot_df):
    ...
    pivot_df['final_composite_score'] = (
        (pivot_df['global_efficiency_index'] * 0.7) + 
        (pivot_df['total_relative_improvement'] * 0.3)
    )
    
    return pivot_df

#  Meraging parental occupation data
def merge_parental_bias(final_df, main_dataset_path):
    ...
    return final_df.merge(
        parents_df[['subject', 'mother_score', 'father_score', 'parental_bias']],
        on='subject', how='left'
    )
